...
def hanoi(n, f, h, t):
    if n == 0:
        ...
        # Moving n-1 disks from to via by using target as an auxiliary position.
        hanoi(n-1, f, t, h)
        move(f, t)
        # Moving n-1 disks from helper position to the target using from position as an auxiliary position.
        hanoi(n-1, h, f, t)
...
def sumAnArray(array):
    ...
    if len(array) == 0:
        ...
        # pop(index) returns element removed and modifies array
        return array.pop(0) + sumAnArray(array)
...
def countItemsInArray(array):
    ...
    if len(array) == 0:
        ...
        # remove(e) returns 'None' where 'e' is element
        return int(0 if array.remove(array[0]) is None else 0) + 1 + countItemsInArray(array)
